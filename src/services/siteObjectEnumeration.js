 ...
 */
async function paginateWithIsLast(url, headers, pageSize, extraParams = {}) {
  const allValues = [];
  let startAt = 0;
 ...
 */
async function enumerateWorkflows(cloudId, accessToken) {
  const url = `${JIRA_API_BASE}/${cloudId}/rest/api/3/workflow/search`;
  const headers = { Authorization: `Bearer ${accessToken}`, Accept: 'application/json' };
  const workflows = await paginateWithIsLast(url, headers, WORKFLOW_PAGE_SIZE);
 ...
 * @returns {Promise<object[]>}
 */
async function enumerateCustomFields(cloudId, accessToken) {
    ...
    await Promise.all(
      chunk.map(async (ctx) => {
        ...
        if (shouldEnumerateOptions) {
          ...
          options = await paginateWithIsLast(optUrl, headers, OPTIONS_PAGE_SIZE, {
            contextId: ctx.id,
          });
        }
  ...
  for (let i = 0; i < customFields.length; i += CONTEXT_OPTIONS_CONCURRENCY) {
    ...
    const results = await Promise.all(
      chunk.map((field) => {
        ...
        return enumerateCustomFieldContexts(cloudId, accessToken, field.id, fieldType);
      })
    );
